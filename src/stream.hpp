...
public:
  ...
  stream<T> cdr() const { return _cdr; }
...
public:
  ...
  T value() const { return _cons->car(); }
...
template <typename T> class stream {
public:
  stream(std::function<std::unique_ptr<const item<T>>()> f)
      : _lazied(std::make_shared<suspension<const item<T>>>(f)) {}
  stream() {}

  T get() const { return _lazied->force().value(); }
